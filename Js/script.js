...
// Lógica de la pantalla de pregunta
...
function moveNoButton() {
  ...
  // Márgenes de seguridad para que siempre sea visible
  const margin = 30;
  
  // Calcular posiciones aleatorias (asegurando que esté completamente visible)
  const minX = margin;
...
// Evento al intentar hacer click en "No" (desktop)
noBtn.addEventListener('click', (e) => {
  e.preventDefault();
  moveNoButton();
});

// Evento al pasar el mouse sobre "No" (hace más difícil atraparlo)
noBtn.addEventListener('mouseenter', moveNoButton);

// Evento al tocar "No" (móvil)
...
yesBtn.addEventListener('click', () => {
  ...
  // Iniciar la animación del árbol después de un breve delay
  setTimeout(() => {
    startTreeAnimation();
  }, 800);
});
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Siempre tuyo, Edu";
...
function showCountdown() {
  ...
  let startDate = startParam ? new Date(startParam + 'T00:00:00') : new Date('2023-08-13T00:00:00'); 
  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('2026-08-13T00:00:00');
  ...
  function update() {
    ...
    container.innerHTML =
      `Hemos vivido juntos <b>${days}</b> días.<br>` +
      `Nuestro aniversario será en <b>${eventDays}d ${eventHours}h ${eventMinutes}m ${eventSeconds}s</b>`;
